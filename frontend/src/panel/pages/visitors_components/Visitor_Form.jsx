...
const VisitorForm = ({ onClose, visitor }) => {
  ...
  useEffect(() => {
    if (visitor) {
      setForm({
        ...
        email: visitor.email || '',
        phone: visitor.mobileNumber || visitor.phone || '',
        status: visitor.status || 'pending',
        organization: visitor.organizationName || visitor.organization || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await apiPut(`/api/visitors/${visitor._id || visitor.id}`, visitorData, token);
        setSuccess('Visitor updated successfully!');
      } else {
        await apiPost('/api/visitors', visitorData, token);
      ...
      setError(err.message || `Failed to ${isEditing ? 'update' : 'register'} visitor. Please try again.`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="rounded-lg border border-gray-700 bg-gray-900 text-gray-100 shadow-lg p-6 space-y-6">
      <div className="flex flex-col space-y-1.5 mb-2">
        ...
        <p className="text-sm text-gray-400">
          {isEditing ? 'Update visitor details' : 'Fill in the details to register a new visitor'}
        </p>
      </div>
      ...
      <div className="flex gap-3 pt-4 justify-end">
        ...
        <Button type="submit" disabled={loading} color="default">
          ...
          {loading ? (isEditing ? 'Updating Visitor...' : 'Creating Visitor...') : (isEditing ? 'Update Visitor' : 'Create Visitor')}
        </Button>
      </div>
    </form>
  );
